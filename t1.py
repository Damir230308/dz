...
def average_earning_per_year():
    my_var = "company"
    n = int(input('Введите кол-во предриятий: '))
    ...
    total_aver = 0
    for val in profit_aver.values():
        total_aver += val
    total_aver = total_aver / n
    for key, val in profit_aver.items():
        if val > total_aver:
            print(f'{key} - ирнинг выше среднего')
        elif val < total_aver:
            print(f'{key} - ирнинг ниже среднего')
        elif val == total_aver:
            print(f'{key} - средний ирнинг')
average_earning_per_year()
